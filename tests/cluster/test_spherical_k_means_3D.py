...
def plot_3d_scatter(
    ...
    if d != 3:
        raise Exception(
            "data should be of shape Nx3, i.e., each data point should be 3D."
        )
...
def test_spherical_k_means():
    ...
    # Save plot instead of showing to avoid blocking pytest
    # Set environment variable SHOW_PLOTS=1 to display plots during development
    import sys
    import os
    ...
    save_or_show_plot("test_spherical_k_means_3d_result.png")
...
def test_SKM_init_spherical_kmpp():
    ...
    skm = SphericalKMeans(
        n_clusters=rand_data.n_components, init="spherical-k-means++", verbose=1
    )
    skm.fit(X)
    print("cluster_centers_", skm.cluster_centers_)
    print("labels_", skm.labels_)
