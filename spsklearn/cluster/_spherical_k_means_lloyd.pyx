...
from ._spherical_k_means_common import CHUNK_SIZE
from ._spherical_k_means_common cimport _relocate_empty_clusters_dense
from ._spherical_k_means_common cimport _relocate_empty_clusters_sparse
from ._spherical_k_means_common cimport _average_centers, _center_shift
...
def lloyd_iter_chunked_dense(
    ...
    """Single iteration of spherical K-means lloyd algorithm with dense input.
...
cdef void _update_chunk_dense(
    ...
    """Spherical K-means combined EM step for one dense data chunk.
    ...
    cdef:
        ...
        floating cos_dist, min_cos_dist
        int i, j, k, label

    ### We only need to store X.C^T 
    ### initialize pairwise_distances=1
    for i in range(n_samples):
        for j in range(n_clusters):
            pairwise_distances[i * n_clusters + j] = 0

    # pairwise_distances = 0*pairwise_distances + X.dot(C.T)
    _gemm(RowMajor, NoTrans, Trans, n_samples, n_clusters, n_features,
          1.0, &X[0, 0], n_features, &centers_old[0, 0], n_features,
          0.0, pairwise_distances, n_clusters)

    for i in range(n_samples):
        min_cos_dist = 1 - pairwise_distances[i * n_clusters]
        label = 0
        for j in range(1, n_clusters):
            cos_dist = 1 - pairwise_distances[i * n_clusters + j]
...
cdef void _update_chunk_sparse(
    ...
    for i in range(n_samples):
        min_cos_dist = max_floating
        label = 0

        for j in range(n_clusters):
            cos_dist = 0.0
            for k in range(X_indptr[i] - s, X_indptr[i + 1] - s):
                cos_dist += centers_old[j, X_indices[k]] * X_data[k]
            # We only need to compute 1 - X.C^T
            cos_dist = 1 - cos_dist
            if cos_dist < min_cos_dist:
                min_cos_dist = cos_dist
                label = j
