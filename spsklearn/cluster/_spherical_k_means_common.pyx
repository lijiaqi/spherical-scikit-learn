...
cdef floating _cosine_dense_dense(
        const floating* a,  # IN
        const floating* b,  # IN
        int n_features,
        bint normalized
) noexcept nogil:
    """Cosine similarity between a dense and b dense"""
    cdef:
        int i
        int n = n_features // 4
    ...
    for i in range(n):
        ...
        a_squared_norm += a[0] * a[0] + a[1] * a[1] + a[2] * a[2] + a[3] * a[3]
        b_squared_norm += b[0] * b[0] + b[1] * b[1] + b[2] * b[2] + b[3] * b[3]
        a += 4
        b += 4
    ...
    return result/sqrt(a_squared_norm * b_squared_norm) if normalized else result


def _cosine_dense_dense_wrapper(
    const floating[::1] a,
    const floating[::1] b,
    bint normalized
):
    """Wrapper of _cosine_dense_dense for testing purpose"""
    return _cosine_dense_dense(&a[0], &b[0], a.shape[0], normalized)


cdef floating _cosine_sparse_dense(
    ...
    """Cosine similarity between a sparse and b dense"""
    cdef:
...
def _cosine_sparse_dense_wrapper(
    ...
    """Wrapper of _cosine_sparse_dense for testing purpose"""
    return _cosine_sparse_dense(
        a_data, a_indices, b, b_squared_norm, normalized)
...
cpdef floating _inertia_dense(
    ...
    Sum of cosine distance between each sample and its assigned center.
    ...
    for i in prange(n_samples, nogil=True, num_threads=n_threads,
        ...
        if single_label < 0 or single_label == j:
            cos_sim = _cosine_dense_dense(&X[i, 0], &centers[j, 0],
                                             n_features, True)
            inertia += (1 - cos_sim) * sample_weight[i]
...
cpdef void _relocate_empty_clusters_dense(
    ...
    cdef:
        ...
        floating[::1] distances = 1 - (np.asarray(X) * np.asarray(centers_old)[labels]).sum(axis=1)
...
cpdef void _relocate_empty_clusters_sparse(
    ...
    for i in range(n_samples):
        j = labels[i]
        distances[i] = 1 - _cosine_sparse_dense(
            X_data[X_indptr[i]: X_indptr[i + 1]],
            X_indices[X_indptr[i]: X_indptr[i + 1]],
...
cdef void _center_shift(
    ...
    for j in range(n_clusters):
        center_shift[j] = 1 - _cosine_dense_dense(
